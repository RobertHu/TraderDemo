...
public class AliveKeeper implements Runnable {
	...
	public AliveKeeper(OutputStream outputStream,LoginInfoManager loginInfoManager){
	...
	public void run(){
		while(true){
			try {
				...
				KeepAliveTimer.DEFALUT_ALIVE_TIMER.add(System.currentTimeMillis() - beginTime);
				...
				String valueString = result.getFirstChildElement(ResponseConstants.SINGLERESULTCONTENTNODENAME_STRING).getValue();
	...
	private SignalObject request() throws IOException, WaitTimeoutException, InterruptedException{
		Element root = new Element(RequestConstants.CommandRootName);
		CommunicationObject request = RequestCommandHelper.NewCommand(this.loginInfoManager.getSession(),
				"KeepAlive", root);
		try {
			...
			this.logger.error("keep alive wait time out "+ request.getInvokeID());
			throw ex;
		}
		catch (Exception e) {
			return null;
